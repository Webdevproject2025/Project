let studentBmiData = []; // Array para sa data ng BMI ng mga estudyante
const BMI_STORAGE_KEY = 'studentBMIList'; // Key para sa local storage
...
function calculateBMI() {
    ...
    if (name && !isNaN(heightCm) && !isNaN(weightKg) && heightCm > 0 && weightKg > 0) {
        ...
        renderBMIList();
        saveBMIData(); // Awtomatikong i-save pagkatapos mag-calculate
...
// Tawagin ang loadBMIData sa unang pag-load para makuha ang saved data
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
        if (document.getElementById('bmi-body')) {
            loadBMIData();
            renderBMIList(); // Render kahit walang loaded data para sa initial view
        }
    });
} else {
    if (document.getElementById('bmi-body')) {
